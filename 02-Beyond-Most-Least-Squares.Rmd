...
##Model 1: Sex Unconditional Model (Any Probability, Independence) and the Principle of Maximum Likelihood {#sex_unconditional_model}
...
From the likelihood in Figure \@ref(fig:lik1), when $p_B$ = 0.3 we see a family of a girl followed by two boys  6.3\% ($0.7*0.3^2$) of the time. However, it indicates that we are much more likely to see our data if $p_B$ = 0.6 where the likelihood of GBB is $0.4*0.6^2$ or 14.4\%.
  
If the choice was between 0.3 and 0.6 for an estimate of $p_B$, we'd choose 0.6. The "best" estimate of $p_B$ would be the value where we are most likely to see our data from all possible values between 0 and 1, which we refer to as the __maximum likelihood estimate__ or MLE. We can approximate an MLE using graphical or numerical approaches. Graphically, here it looks like the MLE is about 0.6. In many, but not all, circumstances, we can obtain an MLE exactly using calculus. In this simple example, the MLE is 2/3. This is consistent with our intuition since 2 out of the 3 children are boys.

---
<p align="center"> __Principle of Maximum Likelihood__ </p>

> Write a likelihood function, $Lik(p_B)$, which gives the likeihood of seeing our data for each possible  value of a parameter for a given model. The __maximum likelihood estimate (MLE)__ is the value of the parameter that yields the largest likelihood.  
---
...
```{r, lik2, fig.align="center",out.width="60%", fig.cap='Likehood function for the data of 2 families (GBB and GGG). The solid line is at the MLE,  ${p}_B=1/3$',echo=FALSE, warning=FALSE, message=FALSE}
BBGGGplot
```
...
Likcomp1 <- c("p~B~","(1-p~B~)","p^2^~B~","p~B~(1-p~B~)","(1-p~B~)p~B~","(1-p~B~)^2^ p~B~","(1-p~B~)p^2^~B~"," ")
Numfamis <- c(6,7,5,4,5,1,2,30)
Likcomp2 <- c("p^6^~B~","(1-p~B~)^7^","p^10^~B~","p^4^~B~(1-p~B~)^4^","(1-p~B~)^5^p^5^~B~","(1-p~B~)^2^p~B~","(1-p~B~)^2^ p^4^~B~"," ")
```
...
\begin{equation}
Lik(p_B) = p_B^{nBoys}(1-p_B)^{nGirls}
(\#eq:lik30v2)
\end{equation}
...
---
<p align="center"> __Approaches to Finding MLEs__ </p>

> 1. Plot the likelihood as a function of possible parameter values. Approximate the MLE by visually determining the value where the  likelihood is a maximum. 

> 2. Use software to  approximate the $p_B$ where the likelihood is a maximum.  

> 3. Use calculus. Find the first derivative of the log-likelihood, set it equal to 0 and solve for the MLE. Check that it is a maximum. (This is optional.)
---
...
\begin{eqnarray*}
Lik(p_B) & = & p_B^{30}(1-p_B)^{20} \\
logLik(p_B) &=& 30log(p_B)+20log(1-p_B) \\
\frac{d}{dp_B} logLik(p_B) &=& \frac{30}{p_B} -\frac{20}{1-p_B} = 0
(\#eq:dlogLik50)
\end{eqnarray*}
...
Before we are mired in notation and calculus, let's think about how these conditional probabilities can be used to describe sex running in families. While we only had one parameter, $p_B$, to estimate in the Sex Unconditional Model, here we have three parameters: $p_{B|N}$, $p_{B|B_{bias}}$, and $p_{B|G_{bias}}$. Clearly if all three of these probabilities are equal, the probability a child is a boy does not depend upon the existing gender composition of the family and there is no evidence of sex running in families. A conditional probability $p_{B|B_{bias}}$ that is larger than $p_{B|N}$  suggests families with more boys are more likely to produce additional boys in contrast to families with equal boys and girls. This finding would support the theory of "boys run in families." An analogous argument holds for girls. In addition, comparisons of $p_{B|B_{bias}}$ and $p_{B|G_{bias}}$ to the parameter estimate $p_B$ from the Sex Unconditional Model may be interesting and can be performed using likelihoods.  
  
While it may seem that including families with a single child (singletons) would not be helpful for assessing whether there is a preponderance of one sex or another in families, in fact singleton families would be helpful in estimating $p_{B|N}$ because singletons join "neutral families."

```{r, include=FALSE}
Comp2 <- c("B","G","BB","BG","GB","GGB","GBB","Total")
Likcont <- c("p~B|N~","1-p~B|N~","p~B|N~p~B|B~~bias~","p~B|N~(1-p~B|B~~bias~)","(1-p~B|N~)p~B|G~~bias~","(1-p~B|N~)(1-p~B|G~~bias~)p~B|G~~bias~","(1-p~B|N~)p~B|G~~bias~p~B|N~"," ")
...
From Table \@ref(tab:table3chp2) note that every first child enters a neutral family with respect to sex composition. It is also clear from this table that there is a probability associated with every child which can be multiplied to obtain likelihood contributions for each sex composition.  As we noted with the Sex Unconditional Model, we can write $p_{G|B_{bias}}= 1- p_{B|B_{bias}}$ and likewise for families with a girl bias; it is unnecessary to have both  $p_{G|B_{bias}}$ and $p_{B|B_{bias}}$ in the likelihood. Table \@ref(tab:table3chp2) should help to give you an idea of how the notation is defined for the Sex Conditional Model, although as is often the case in statistical modeling, there are other ways to set up models to test for Sex Bias.  
...
\begin{eqnarray*}
log\textrm{-}lik(p_{B|N},p_{B|B_{bias}}, p_{B|G_{bias}})& = &17logp_{B|N}+15log(1-p_{B|N})+5logp_{B|B_{bias}} \\
& &+4log(1-p_{B|B_{bias}}) 
+8logp_{B|G_{bias}}+1log(1-p_{B|G_{bias}}) \\
\end{eqnarray*}
    
Taking a partial derivative with respect to $p_{B|N}$
...
kable(table5chp2, booktabs=T, caption="Number of families and children in families with given composition. Sex ratio and proportion males by family size.")
```
...
&= & p_B^{930+2*582+666+666+\cdots+125}(1-p_B)^{951+666+666+2*530+\cdots+3*159} \\
Lik(p_B) & =&  p_B^{5416}(1-p_B)^{5256} \\
(\#eq:likSexUn)
\end{eqnarray*}
...
> G $(1-p_{B|N})^{951}$  
> GB  $(1-p_{B|N})^{666}p_{B|G_{bias}}^{666}$  
> BGB $p_{B|N}^{2*148}(1-p_{B|B_{bias}})^{148}$  
    
The products of the last three columns of Table \@ref(tab:table8chp2) provide the likelihood contributions for the Sex Conditional Model for all of the one-, two- and three-child NLSY families. We write the likelihood as a function of the three parameters $p_{B|N},  p_{B|B_{bias}}$, and $p_{B|G_{bias}}$.
...
SexUnc <- c("p~B~","(1-p~B~)","p~B~^2^","p~B~ (1-p~B~)","(1-p~B~)p~B~","(1-p~B~)^2^","p~B~^3^", "p~B~^2^ (1-p~B~)","p~B~ (1-p~B~)^2^","p~B~^2^ (1-p~B~)","p~B~^2^ (1-p~B~)","p~B~ (1-p~B~)^2^","p~B~(1-p~B~)^2^", "(1-p~B~)^3^","-7396.067","14794.13","14810.68")
Child1 <- c("p~B|N~"," 1-p~B|N~","p~B|N~","p~B|N~","1-p~B|N~","1-p~B|N~","p~B|N~","p~B|N~","p~B|N~","p~B|N~","1-p~B|N~","1-p~B|N~","1-p~B|N~","1-p~B|N~"," "," "," ")
Child2 <- c(" "," ","p~B|B~~bias~","1-p~B|B~~bias~","p~B|G~~bias~","1-p~B|G~~bias~","p~B|B~~bias~","p~B|B~~bias~","1-p~B|B~~bias~","1-p~B|B~~bias~","p~B|G~~bias~","1-p~B|G~~bias~","p~B|G~~bias~","1-p~B|G~~bias~","-7374.238","14751.48","14749.18")
Child3 <- c(" "," "," "," "," "," ","p~B|B~~bias~","1-p~B|B~~bias~","1-p~B|N~","p~B|N~","p~B|N~","p~B|G~~bias~","1-p~B|N~","1-p~B|G~~bias~"," "," "," ")
```
...
\begin{eqnarray}
Lik(p_{B|N},  p_{B|B_{bias}}, p_{B|G_{bias}})& =& p_{B|N}^{930}(1-p_{B|N})^{951} ( p_{B|N}p_{B|B_{bias}})^{582}(p_{B|N}(1-p_{B|B_{bias}}))^{666}\\
&& \cdots  ((1-p_{B|N})(1-p_{B|G_{bias}})(1-p_{B|G_{bias}}))^{159}\\ 
&=& p_{B|N}^{3161}(1-p_{B|N})^{3119}p_{B|B_{bias}}^{1131}(1-p_{B|B_{bias}})^{1164}p_{B|G_{bias}}^{1124}(1-p_{B|G_{bias}})^{973} \\
(\#eq:likall)
\end{eqnarray}
    
\begin{eqnarray}
logLik(p_{B|N},  p_{B|B_{bias}}, p_{B|G_{bias}})&=& 3161logp_{B|N}+3119log(1-p_{B|N})+\\  & 
 &1131log(p_{B|B_{bias}})+1164log(1-p_{B|B_{bias}})\\   
& &+1124logp_{B|G_{bias}}+ 973log(1-p_{B|G_{bias}})\\
(\#eq:log-likall) 
\end{eqnarray}
    
To use calculus to estimate the probability of a boy entering a neutral family (a family with equal boys and girls), $p_{B|N}$, we begin with the  logarithm of the likelihood in equation \@ref(eq:log-likall). Differentiating the log-likelihood with respect to $p_{B|N}$ holding all other parameters constant yields an intuitive estimate.
...
Similarly, MLEs for $p_{B|B_{bias}}$ and $p_{B|G_{bias}}$ can be obtained:
\begin{eqnarray*}
\hat{p}_{B|B_{bias}}&=& \frac{1131}{1131+1164}\\
& = & 0.4928 \\
& & \\
\hat{p}_{B|G_{bias}}&=&\frac{1124}{1124+973}\\
&=& 0.5360
\end{eqnarray*}
...
$Ho: p_{B|N}=p_{B|B_{bias}}=p_{B|G_{bias}}=p_B$ (Sex Unconditional Model)
The probability of a boy does not depend on the prior family composition.
    
$Ha: \textrm{At least one parameter from } p_{B|N}, p_{B|B_{bias}},p_{B|G_{bias}}$  differs from the others. (Sex Conditional Model)
...
\begin{eqnarray*}
logLik(0.5075)& = &5416*log(.5075) + 5256* log(1-.5075)\\
&=& -7396.067\\
\end{eqnarray*}
    
The maximum of the log-likelihood for the larger model can be found by replacing $p_{B|N},  p_{B|B_{bias}}, p_{B|G_{bias}}$ in the log-likelihood with 0.5033, 0.4928, and 0.5360, respectively.
    
\begin{eqnarray*}
logLik(0.5033, 0.4928, 0.5360) &= & 3161*log(.5033)+3119*log(1-.5033)+\\
...
\[ LRT = 2[maxlogLik(\textrm{larger model}) - maxlogLik(\textrm{reduced model})]\]
\[LRT = 2log\left(\frac{maxLik(\textrm{larger  model})}{maxLik(\textrm{reduced model})} \right)\]
...
*Also note: * It might have been more logical to start by using Likelihood Ratio Test determine whether the probability of having a boy differs significantly from 0.5.  We leave this as an exercise.
...
> $p_{S|B1}=$     probability of stopping after the first boy

> $1 - p_{S|B1}=$ probability of not stopping after the first boy

> $p_{S|N}=$      probability of stopping after a child who is not the first boy

> $1 - p_{S|N}=$  probability of not stopping after a child who is not the first boy

Our interest centers on whether the probability of stopping after the first boy, $p_{S|B1}$ is greater than stopping when it is not a first boy, $p_{S|N}$.
...
Lik <- c("p~B~","(1-p~B~)","p~B~^2^","p~B~ (1-p~B~)","(1-p~B~)p~B~","(1-p~B~)^2^","p~B~^3^", "p~B~^2^ (1-p~B~)","p~B~ (1-p~B~)^2^","p~B~^2^ (1-p~B~)","p~B~^2^ (1-p~B~)","p~B~ (1-p~B~)^2^","p~B~(1-p~B~)^2^", "(1-p~B~)^3^")
Cont <- c("p~S|B1~","p~S|N~","(1-p~S|B1~) p~S|N~","(1-p~S|B1~) p~S|N~","(1-p~S|N~) p~S|B1~", "(1-p~S|N~)p~S|N~","(1-p~S|B1~) (1-p~S|N~)p~S|N~","(1-p~S|B1~) (1-p~S|N~)p~S|N~","(1-p~S|B1~) (1-p~S|N~)p~S|N~","(1-p~S|B1~) (1-p~S|N~)p~S|N~","(1-p~S|N~) (1-p~S|B1~)p~S|N~","(1-p~S|N~)^2^ p~S|B1~","(1-p~S|N~) (1-p~S|B1~)p~S|N~","(1-p~S|N~)^2^ p~S|N~")
```

```{r, table9chp2, echo=FALSE}
#StopProbs.tab
table9chp2 <- data.frame(Famcomp2,Numfamis3,Lik,Cont)
colnames(table9chp2) <- c("Family Composition","Number of families","   Likelihood","Contribution")
...
kable(tab1chp2, booktabs=T, caption="Measures of model performance: Waiting for a Booy vs. Waiting for a Girl Model.")
```
...
We can perform a LRT to compare stopping at the first boy to a Random Stopping Model. The parameters for the first model(waiting for a boy) are $p_B, p_{S|B1}, p_{S|N}$ and the parameters for the second model(random stopping) are $p_B$ and $p_S$. The results suggest that the Waiting for a Boy Model is significantly better than the Random Stopping Model.  The Random Stopping Model takes into account that the odds of stopping after a child are not 50-50, but may be closer to the MLE for $p_S$ of 53. (=52.7\%). We leave the derivation of this result as an exercise.  
